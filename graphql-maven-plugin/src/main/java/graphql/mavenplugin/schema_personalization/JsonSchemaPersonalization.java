...
public class JsonSchemaPersonalization {
	...
	/** If the file in schemaPersonalizationFile is this constant, it means that there is no personalization to apply */
		...
		try {
			...
			if (getSchemaPersonalization() != null) {
				...
				for (EntityPersonalization objectPers : schemaPersonalization.getEntityPersonalizations()) {
					...
					for (graphql.mavenplugin.schema_personalization.Field field : objectPers.getNewFields()) {
					...
					for (graphql.mavenplugin.schema_personalization.Field field : objectPers
	...
	public SchemaPersonalization loadGraphQLSchemaPersonalization() throws IOException, URISyntaxException {
		...
		if (schemaPersonalizationFile.getName().equals(NO_PERSONALIZATION)) {
			return null;
		} else {
			...
			nbErrors = 0;
			try (JsonReader reader = service.createReader(new FileInputStream(schemaPersonalizationFile), schema,
					handler)) {
				// JsonValue value =
				reader.readValue();
			...
			if (nbErrors > 0) {
				throw new RuntimeException("The json file '" + schemaPersonalizationFile.getAbsolutePath()
			...
			// schema.validate(new JSONObject(new FileInputStream(schemaPersonalizationFile)));
			...
			log.info("Loading file " + schemaPersonalizationFile.getAbsolutePath());
			ObjectMapper objectMapper = new ObjectMapper();
			SchemaPersonalization ret;
			try (InputStream isFlowJson = new FileInputStream(schemaPersonalizationFile)) {
