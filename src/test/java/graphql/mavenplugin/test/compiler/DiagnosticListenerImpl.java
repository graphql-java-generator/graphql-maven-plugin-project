...
public class DiagnosticListenerImpl implements DiagnosticListener<JavaFileObject> {
	 ...
	 * @param log
	 */
	public DiagnosticListenerImpl(Log log) {
		this.log = log;
	}
	 ...
	 * @param className
	 */
	public DiagnosticListenerImpl(Log log, String className) {
		this.log = log;
	...
	public void report(Diagnostic<? extends JavaFileObject> diagnostic) {
		// In debug mode, we show all messages, but in standard mode we show only Errors and Mand
		if (diagnostic.getKind().equals(Diagnostic.Kind.ERROR)
				|| diagnostic.getKind().equals(Diagnostic.Kind.MANDATORY_WARNING)) {
			if (log.isErrorEnabled()) {
				log.error(logDiagnostic(diagnostic));
			}
	 ...
	 * @param diagnostic
	 */
	String logDiagnostic(Diagnostic<? extends JavaFileObject> diagnostic) {
