 ...
 * @author EtienneSF
 */
public class ObjectResponseOld {
	 ...
	 * own sub-objects
	 */
	List<ObjectResponseOld> subObjects = new ArrayList<>();
	 ...
	 */
	ObjectResponseOld(Class<?> owningClass, String fieldName) throws GraphQLRequestPreparationException {
	 ...
	 * @see #newResponseDefBuilder(String)
	 */
	ObjectResponseOld(Class<?> owningClass, String fieldName, String fieldAlias)
			throws GraphQLRequestPreparationException {
	...
	public void appendResponseQuery(StringBuilder sb, Map<String, Object> parameters, boolean appendSpaceParam)
		...
		if (scalarFields.size() > 0 || subObjects.size() > 0) {
			...
			for (ObjectResponseOld o : subObjects) {
