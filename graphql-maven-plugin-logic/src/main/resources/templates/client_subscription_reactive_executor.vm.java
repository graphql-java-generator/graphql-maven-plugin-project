...
#end
 * 
 * This class contains the methods that allows the execution of the subscriptions that are defined in the ${object.name} 
 * of the GraphQL schema. All the methods for this executor are in spring reactive, that is: they return a {@link Flux}.<BR/>
...
public class ${object.name}ReactiveExecutor${springBeanSuffix}  implements GraphQLSubscriptionReactiveExecutor {
...
## The @..@ is the placeholder for the maven resource filtering
	...
	/**
	 * This method takes a subscription <B>full request</B> definition, and executes it <b>in reactive mode</b> against the 
	 * GraphQL server. It may <B>only return one subscription's field</B>.<BR/>
	 ...
	 *          
	 *          Flux<${subscriptionType.classFullName}> flux = subscriptionExecutor.execWithBindValues(
	 *              "subscription { sampleSubscriptionField(param: ?param)  {subfield1 @skip(if: &skip) subfield2 {id name}}}",
	 *              callback,
	 *              params);
	 *          
	 *          ... do something with this Flux
	 *     }
	 * }
	 * </PRE>
	 * 
	 ...
	 *     
	 *      void myMethod() {
	 *          Flux<${subscriptionType.classFullName}> flux = subscriptionExecutor.exec(
	 *              "subscription { sampleSubscriptionField(param: ?param)  {subfield1 @skip(if: &skip) subfield2 {id name}}}",
	 *              callback,
...
#foreach ($line in $field.description.lines)
...
#end
	 *
	 * This method registers a subscription, by executing a direct partial ${field.name} subscription against the GraphQL server. This
	 ...
	 *          params.put("skip", Boolean.FALSE); // skip is mandatory, as it is marked by a "&" in the request
	 *          
	 *          Flux<Optional<${field.javaTypeFullClassname}>> flux = subscriptionExecutor.${field.javaName}(
	 *              "{subfield1(param: ?param) @skip(if: &skip) subfield2 {id name}}",
	 *              callback,
#foreach ($inputParameter in $field.inputParameters)
...
#appliedDirectives(${field.appliedDirectives}, "	")
	public Flux<Optional<${field.javaTypeFullClassname}>> ${field.javaName}WithBindValues(
			String queryResponseDef,
#inputParams()
			Map<String, Object> parameters)
			throws GraphQLRequestExecutionException, GraphQLRequestPreparationException {
...
#foreach ($inputParameter in $field.inputParameters)
	...
	* @param ${inputParameter.name} Parameter for the ${field.name} field of ${object.name}, as defined in the GraphQL schema
	 ...
	 */
#appliedDirectives(${field.appliedDirectives}, "	")
	public Flux<Optional<${field.javaTypeFullClassname}>> ${field.javaName}(String queryResponseDef,
#inputParams()
			Object... paramsAndValues)
			throws GraphQLRequestExecutionException, GraphQLRequestPreparationException {
...
#foreach ($line in $field.description.lines)
	 ...
	 *         // Preparation of the query, so that it is prepared once then executed several times
	 *         preparedRequest = subscriptionExecutor
	 *             .get${field.pascalCaseName}GraphQLRequest("{subfield1(param: ?param) @skip(if: &skip) subfield2 {id name}}");
	 *     }
	 *     
	 *      void myMethod() {
	 * 	        Map<String, Object> params = new HashMap<>();
	 ...
	 *          params.put("skip", Boolean.FALSE); // skip is mandatory, as it is marked by a "&" in the request
	 *          
	 *          Flux<Optional<${field.javaTypeFullClassname}>> flux = subscriptionExecutor.${field.javaName}WithBindValues(
	 *              preparedRequest,
	 *              callback,
#foreach ($inputParameter in $field.inputParameters)
	 *              ${inputParameter.name}, // A value for ${field.name}'s ${inputParameter.name} input parameter
...
#appliedDirectives(${field.appliedDirectives}, "	")
	...
	@SuppressWarnings("static-method")
#end
	public Flux<Optional<${field.javaTypeFullClassname}>> ${field.javaName}WithBindValues(ObjectResponse objectResponse,
#inputParams() 
			Map<String, Object> parameters)
			throws GraphQLRequestExecutionException  {
		if (logger.isTraceEnabled()) {
...
#end 
	public Flux<Optional<${field.javaTypeFullClassname}>> ${field.javaName}(
			ObjectResponse objectResponse,
#inputParams() 
			Object... paramsAndValues)
			throws GraphQLRequestExecutionException  {
